...
2. **KPI cards (6, matching PLAN §4):** OTA % · Avg delay + reason mix (late-only shares) · No-show % · Cost/trip (+ cost/km + zero-km share) · Alert rate/1k (+ Sev-1 + ack SLA share) · CSAT (+ low-rating share). Each card: current value, Δ vs prior cycle, **benchmark badge** (✓ ≥ SLA green / ✗ breach red: OTA 95%, ack 30 min), peer context ("rank #3/23 vendors").
