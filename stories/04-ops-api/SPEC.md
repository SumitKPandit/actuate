...
**Out:** narration text (Story 07 — endpoints return facts + template fallback string only), frontend (Stories 05–06), `/ask` full NL→SQL (Story 07; reserve path here with 501 or minimal stub).
...
3. `GET /briefing?cycle=` → `{ generated_at, headline_facts[3-5 strings, template-rendered], insights_top5, safety_open_sev1, actions_top3 }`. No LLM call here (Story 07 adds optional `?narrate=true` later). Cached in `insight_cache` under `briefing:{cycle}`.
...
5. `GET /actions?cycle=` → flattened `{ action, owner, due_hint, copy_for_vendor }` derived from insights (copy text is template fill, ≤ 500 chars, includes vendor name + KPI + cycle).
6. Errors: unknown cycle → 404 with valid cycle list; empty mart → 200 with `data: null, warning: "marts empty — run ingest"`. Never 500 on empty data.
