...
2. **Delay:** `avg_delay_min` (mean of `delay_minutes`), plus `reason_mix` counts/share over `{NODELAY, TRAFFIC, DRIVER, EMPLOYEE}`. Vendor/office attribution = group-by caller provides, function supports `group_key` param.
