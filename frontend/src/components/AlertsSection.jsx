    ...
    <div
      ...
      className={`bg-surface rounded-lg border shadow-sm p-5 flex flex-col gap-4 transition alert-card ${
        highlighted
          ...
          : 'border-border-light hover:border-neutral-muted/40'
      } ${hidden ? 'hidden' : ''}`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wider ${sevBadge[alert.severity]}`}
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${alert.severity === 'high' ? 'bg-error' : alert.severity === 'medium' ? 'bg-warning' : 'bg-neutral-muted'} ${alert.severity === 'high' && alert.id === 2 ? 'animate-ping' : ''}`}
            ></span>
            {alert.severity.toUpperCase()} SEVERITY
          </span>
          <span className="text-base font-bold text-neutral-title">
            {alert.title}
          </span>
        </div>
        <div className="flex items-center gap-1.5 text-xs">
          <span className="text-neutral-muted">{alert.scopeLabel}:</span>
          <span className="font-semibold text-neutral-title bg-surface-panel border border-border-light px-2 py-0.5 rounded-full">
            {alert.scope}
          </span>
        </div>
      </div>
      ...
      <div
        className={`grid grid-cols-1 gap-4 bg-surface-panel p-4 rounded-lg border border-border-light ${alert.metrics.length === 4 ? 'md:grid-cols-4' : 'md:grid-cols-3'}`}
      >
        {alert.metrics.map((m, i) => (
          <div key={i}>
            <span className="text-[11px] font-bold text-neutral-muted uppercase tracking-wider">
              {m.label}
            </span>
                ...
                className={`${m.bigSmall ? 'text-xs font-semibold' : 'text-2xl font-bold'} ${
                  m.id === 'alert2-status'
                    ? acked
                      ? 'text-primary'
                      : 'text-warning'
                    : bigTone[m.bigTone] || bigTone.default
                }`}
              >
                {m.id === 'alert2-status' ? ackTier : m.big}
              </span>
              {m.small && (
                <span className="text-xs text-neutral-muted">{m.small}</span>
              )}
              {m.inlineNote && (
                <span className="text-xs text-error font-semibold">
                  {m.inlineNoteId === 'card2-unack-text'
                    ? unackText
                    : m.inlineNote}
                </span>
              )}
            ...
            {m.note && (
              <span
                className={`text-xs ${noteTone[m.noteTone] || noteTone.default}`}
              >
                {m.note}
              </span>
            )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-1 gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[11px] uppercase font-bold text-neutral-muted">
            Recommended Action:
          </span>
          <span className="text-xs font-medium text-neutral-body">
            {alert.recommended}
          </span>
          <span
            className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${alert.ownerTone === 'error' ? 'bg-error-bg text-error border-error/20' : 'bg-blue-50 text-secondary border-secondary/20'}`}
          >
            Owner: {alert.owner}
          </span>
        </div>
          ...
          {alert.hasAckButton ? (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-bold shadow-sm transition flex items-center gap-1 ${
                acked
                  ? 'bg-surface-panel border border-border-light text-neutral-title cursor-default'
            ...
            <button
              ...
              onClick={() =>
                onApproveDirect(
                  alert.actionTitle || alert.actionLabel,
                  alert.badgeId,
                )
              }
            >
          ...
          <div className="bg-surface-panel p-4 rounded-lg border border-border-light flex flex-col gap-3">
            <div className="flex items-center justify-between pb-2 border-b border-border-light">
              ...
              <span className="text-[11px] font-semibold text-primary">
                High Confidence Model ({alert.breakdown.confidence})
              </span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              {alert.breakdown.rows.map(([label, val], i) => (
                <div key={i}>
                  ...
                  <strong
                    className={
                      val.includes('93.0') || val.includes('42%')
                        ? 'text-error'
                        : ''
                    }
                  >
            ...
            <p className="text-xs text-neutral-muted pt-2 border-t border-border-light leading-relaxed">
              <strong>AI Reasoning &amp; Direct Action:</strong>{' '}
              {alert.breakdown.note}
            </p>
          </div>
        </div>
      )}
    ...
    <div className="flex flex-col gap-4" id="priority-alerts-section">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1">
        <div className="flex flex-col">
          <h3 className="text-lg font-bold text-neutral-title tracking-tight">
            PRIORITY ALERTS
          </h3>
          <span className="text-xs text-neutral-muted">
            AI-ranked exceptions requiring operational attention.
          </span>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto">
            ...
            {['all', 'critical', 'high', 'medium', 'low'].map((sev) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold transition ${
                  filter === sev
                    ? 'bg-secondary text-surface shadow-2xs'
                    : 'font-semibold text-neutral-muted hover:text-neutral-title'
                }`}
          ...
          <div className="hidden md:flex items-center gap-1.5 bg-surface px-3 py-1.5 rounded-full border border-border-light shrink-0">
            <span className="text-[11px] text-neutral-muted uppercase font-semibold">
              Sort:
            </span>
