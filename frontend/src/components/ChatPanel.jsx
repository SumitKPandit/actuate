  ...
  return (
    <div className="bg-surface-panel p-3.5 rounded-lg rounded-tl-none border border-border-light flex flex-col gap-3 text-neutral-body">
      <p className="text-xs leading-relaxed">
        OTA fell to <span className="font-bold text-error">93.0%</span>, which is 2.0pp below the{' '}
        <span className="font-semibold text-neutral-title">95.0% SLA</span>. <span className="font-bold text-secondary">Vendor X</span>{' '}
        contributed <span className="font-bold text-error">42% of the delay gap</span>, followed by{' '}
        <span className="font-semibold text-neutral-title">Vendor Y</span> at 21%.
      </p>
      <div className="overflow-hidden rounded-lg border border-border-light bg-surface">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-border-light font-mono text-[11px]">
            <tr>
              <td className="px-3 py-2 font-semibold text-neutral-title">Vendor X</td>
              <td className="px-3 py-2 text-error font-bold">91.2%</td>
              <td className="px-3 py-2 text-error font-semibold">42%</td>
            </tr>
            <tr>
              <td className="px-3 py-2 font-semibold text-neutral-title">Vendor Y</td>
              <td className="px-3 py-2 text-neutral-body">93.1%</td>
              <td className="px-3 py-2 text-neutral-body">21%</td>
            </tr>
            <tr>
              <td className="px-3 py-2 font-semibold text-neutral-title">Others (FleetOps)</td>
      ...
      <div className="flex flex-col gap-1 pt-1 border-t border-border-light">
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-neutral-muted flex items-center gap-1">
            ...
            Grounded from: <code className="font-mono text-neutral-title font-medium">daily_kpi, vendor_kpi, insight_cache</code>
          </span>
          <button className="text-[11px] text-secondary font-semibold hover:underline" onClick={() => setSqlOpen((v) => !v)}>
...
LIMIT 50;`}</pre>
      ...
      <div className="flex items-center justify-between pt-1">
        <span className="text-[11px] text-neutral-muted">Recommended Next Step:</span>
        <button
...
function RoutesResponseBody() {
  ...
  return (
    <div className="bg-surface-panel p-3.5 rounded-lg rounded-tl-none border border-border-light flex flex-col gap-3 text-neutral-body">
      <p className="text-xs leading-relaxed">
        <span className="font-bold text-neutral-title">Route 42</span> and <span className="font-bold text-neutral-title">Route 18</span>{' '}
        account for the largest share of delayed trips associated with Vendor X.
      </p>
      <div className="overflow-hidden rounded-lg border border-border-light bg-surface">
        <table className="w-full text-left text-xs">
          <thead className="bg-surface-panel text-[11px] font-bold text-neutral-muted uppercase border-b border-border-light">
          ...
          <tbody className="divide-y divide-border-light font-mono text-[11px]">
            <tr>
              <td className="px-3 py-2 font-semibold text-neutral-title">Route 42</td>
            ...
            <tr>
              <td className="px-3 py-2 font-semibold text-neutral-title">Route 18</td>
      ...
      <div className="flex flex-col gap-1 pt-1 border-t border-border-light">
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-neutral-muted flex items-center gap-1">
            <span className="material-symbols-outlined text-[14px] text-primary">verified</span>
            Grounded from: <code className="font-mono text-neutral-title font-medium">mart.route_telemetry, trip_corridors</code>
          </span>
          <button className="text-[11px] text-secondary font-semibold hover:underline" onClick={() => setSqlOpen((v) => !v)}>
            {sqlOpen ? 'Hide query' : 'View query'}
          </button>
...
function Sev1ResponseBody({ unackCount, onOpenSafety }) {
  return (
    <div className="bg-surface-panel p-3.5 rounded-lg rounded-tl-none border border-border-light flex flex-col gap-2.5 text-neutral-body">
      <p className="text-xs leading-relaxed">
        Currently tracking <span className="font-bold text-error">18 Open Sev-1 alerts</span> across central dispatch.{' '}
        <span className="font-bold text-error">{unackCount} incidents are unacknowledged</span> past the 30-minute operational threshold.
      </p>
      <div className="flex items-center gap-2 pt-1">
        <button onClick={onOpenSafety} className="px-3.5 py-1.5 rounded-full bg-secondary text-surface text-xs font-bold hover:bg-primary-hover transition">
...
function CostResponseBody() {
  return (
    <div className="bg-surface-panel p-3.5 rounded-lg rounded-tl-none border border-border-light flex flex-col gap-2 text-neutral-body">
      <p className="text-xs leading-relaxed">
        <span className="font-bold text-secondary">Vendor Y</span> has 14 extreme billing lines exceeding the ₹16,000 threshold (+3.2σ
        outlier vs baseline ₹1,236). Recommend placing a hold on unslabbed km lines.
      </p>
    </div>
  )
}
...
function NoShowResponseBody() {
  return (
    <div className="bg-surface-panel p-3.5 rounded-lg rounded-tl-none border border-border-light flex flex-col gap-2 text-neutral-body">
      <p className="text-xs leading-relaxed">
        <span className="font-bold text-neutral-title">Office B (Tech Hub)</span> has the highest no-show rate at{' '}
        <span className="font-bold text-warning">4.1%</span> (vs corporate peer average 2.3%). Concentrated along late evening shifts.
      </p>
    </div>
  )
}
  ...
  return (
    <div className="bg-error-bg/60 border border-error/30 p-4 rounded-lg flex flex-col gap-2.5">
      <div className="flex items-center gap-1.5 text-error">
        ...
        <span className="text-xs font-bold uppercase">422 — Intent Out of Scope</span>
      </div>
      <p className="text-xs text-neutral-body leading-relaxed">Actuate couldn't answer "{query}".</p>
      <div className="flex flex-col gap-1 bg-surface p-3 rounded-lg border border-border-light">
        <span className="text-[11px] font-bold text-neutral-muted uppercase">I can help with:</span>
...
function classify(query) {
  ...
  if (lower.includes('weather') || lower.includes('driver phone') || lower.includes('pii') || lower.includes('joke')) return 'fallback'
  if (lower.includes('route') || lower.includes('corridor')) return 'routes'
  if (lower.includes('sev-1') || lower.includes('safety') || lower.includes('incident')) return 'sev1'
...
export default function ChatPanel({ unackCount, onOpenSafety, onCopyVendor }) {
  const [turns, setTurns] = useState([{ type: 'ota-default', user: 'Why did OTA drop?', ts: '06:31 IST' }])
  ...
  const renderTurn = (turn, idx) => {
        ...
        <React.Fragment key={idx}>
          ...
          <div className="flex flex-col gap-1.5 max-w-[95%]">
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] font-bold text-secondary">ACTUATE CO-PILOT</span>
    ...
    const bodies = {
      ...
      sev1: <Sev1ResponseBody unackCount={unackCount} onOpenSafety={onOpenSafety} />,
  ...
  return (
    <div className="xl:col-span-4 flex flex-col gap-4 sticky top-20">
      <div className="bg-surface rounded-lg border border-border-light shadow-sm flex flex-col h-[calc(100vh-6rem)] overflow-hidden">
        ...
        <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-4" ref={streamRef}>
          {!fallbackPreview ? (
            <>
              <div className="bg-surface-panel p-3.5 rounded-lg border border-border-light flex flex-col gap-2">
                <span className="text-[11px] font-bold text-secondary uppercase tracking-wider">Operations Synthesis</span>
                <p className="text-xs text-neutral-body leading-relaxed">
                  Your operation has <span className="font-bold text-error">5 priority exceptions</span>. I can explain what changed,
                  identify root causes, and prepare vendor communications.
                </p>
              ...
              {loading && (
                <div className="flex items-center gap-2 text-xs text-neutral-muted py-2 px-1">
                  ...
                  <span className="italic">Actuate is analyzing operational data from marts...</span>
                </div>
              )}
            </>
          ) : (
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-error-bg/60 border border-error/30 p-4 rounded-lg flex flex-col gap-3">
                ...
                <p className="text-xs text-neutral-body leading-relaxed">Actuate couldn't answer that question. I can help with:</p>
                <div className="flex flex-col gap-1 bg-surface p-3 rounded-lg border border-border-light">
                  <span className="text-[11px] font-bold text-neutral-muted uppercase">I am grounded to answer:</span>
        ...
        <div className="p-3.5 bg-surface border-t border-border-light flex flex-col gap-1.5">
          ...
          <div className="flex items-center justify-between px-1 text-[11px] text-neutral-muted">
            ...
            <span className="hover:text-secondary cursor-pointer font-medium" onClick={() => submit('Which routes are affected?')}>
