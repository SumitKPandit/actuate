  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        <span className="text-xs font-semibold text-surface">{toast?.title || ''}</span>
        <p className="text-xs text-neutral-muted leading-tight line-clamp-3 text-gray-200">{toast?.message || ''}</p>
      </div>
    </div>
  )
}
