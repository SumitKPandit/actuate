      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-xs text-neutral-muted">
            Decisions ready for human approval.
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {actions.map((action) => {
          ...
          return (
            <div
              key={action.id}
              className="bg-surface p-4 rounded-lg border border-border-light shadow-sm flex flex-col justify-between gap-4"
            >
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span
                    className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${
                      approved
                        ? 'bg-success-bg text-primary border-primary/20'
                        : 'bg-surface-panel border-border-light text-neutral-muted'
                    }`}
                  >
                    {approved ? '✓ ACKED' : 'PROPOSED'}
                  </span>
                  <span
                    className={`text-xs font-semibold ${action.ownerTone === 'error' ? 'text-error' : 'text-secondary'}`}
                  >
                    Owner: {action.owner}
                  </span>
                </div>
                <h4 className="text-sm font-bold text-neutral-title mt-1">
                  {action.title}
                </h4>
                ...
                {approved && (
                  <span className="text-[11px] text-primary font-medium">
                    Acknowledged just now (Operator: Transport Manager)
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-border-light">
                <button
                  ...
                  onClick={() =>
                    approved
                      ? onViewAudit(action.title)
                      : onApprove(action.id, action.title)
                  }
                >
                  {approved && (
                ...
                {action.hasCopy && (
                  <button
                    ...
                    onClick={() =>
                      onCopyVendor(action.vendorName, action.vendorOta)
                    }
